...
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from "@/components/ui/form"
import {
  rendezVousPersonalSchema,
  rendezVousMeetingSchema,
  rendezVousValidationSchema,
  type RendezVousFormValues,
} from "@/lib/schemas"
...
export default function RendezVousForm({
  ...
  const form = useForm<RendezVousFormValues>({
    resolver: zodResolver(
      rendezVousPersonalSchema.merge(rendezVousMeetingSchema).merge(rendezVousValidationSchema)
    ),
    defaultValues: {
      nom: "",
      prenom: "",
  ...
  const handleNext = async () => {
    let isValid = false
    if (currentStep === 0) {
      isValid = await form.trigger(Object.keys(rendezVousPersonalSchema.shape) as (keyof RendezVousFormValues)[])
    } else if (currentStep === 1) {
      isValid = await form.trigger(Object.keys(rendezVousMeetingSchema.shape) as (keyof RendezVousFormValues)[])
    } else if (currentStep === 2) {
      isValid = await form.trigger(Object.keys(rendezVousValidationSchema.shape) as (keyof RendezVousFormValues)[])
    }
  ...
  const handleSubmitForm = (data: RendezVousFormValues) => {
    onSubmit(data, files)
  }
