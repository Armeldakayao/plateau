...
import { useDeleteDocument, useUserFiles } from "@/hooks/uploads/use-upload-mutations"
import { DocumentIcon } from "./document-icon"
import { DocumentUploadDialog } from "./document-upload"
import { DocumentEditDialog } from "./edit-document-dialog"
...
export function DocumentList({ showAll = false, canManage = true, maxItems = 5 }: DocumentListProps) {
  ...
  const { data: filesData, isLoading, refetch } = useUserFiles("n")
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {canManage && (
            <Button size="sm" onClick={() => setIsUploadOpen(true)} className="flex items-center gap-2">
