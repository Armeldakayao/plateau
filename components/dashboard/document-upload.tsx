...
import { useMyDocuments, useUploadDocument, useUserFiles } from "@/hooks/uploads/use-upload-mutations"
...
export function DocumentUploadDialog({ open, onOpenChange, onSuccess }: DocumentUploadDialogProps) {
  ...
  const { data: existingFiles } = useMyDocuments("n")
  const { toast } = useToast()
 //@ts-ignore
  const existingDocumentTypes = existingFiles?.map((doc) => doc.documentType) || []
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Sélectionnez un fichier et renseignez les informations du document.</DialogDescription>
        </DialogHeader>
